...
/** Minimal line icons keyed by service index. */
const ICONS = [
  // roof
...
/** 4 — Services. Elegant cards with iconography + hover detail. */
export function Services() {
  return (
    <section id="services" className="relative bg-void-2 py-32 md:py-44">
      <div className="shell">
        <Reveal>
          <Kicker>{SERVICES.kicker}</Kicker>
        </Reveal>
        <Reveal delay={0.05}>
          <h2 className="mt-8 max-w-3xl text-display-sm font-semibold text-paper">{SERVICES.headline}</h2>
        </Reveal>
          ...
          {SERVICES.items.map((s, i) => (
            <Reveal key={s.title} delay={0.04 * i} className="h-full">
              <article className="group relative h-full bg-void p-8 transition-colors duration-500 hover:bg-ink">
                <div
                  aria-hidden
                  className="pointer-events-none absolute inset-x-0 top-0 h-px scale-x-0 bg-cyan transition-transform duration-500 ease-cine group-hover:scale-x-100"
                />
                <svg
                  viewBox="0 0 24 24"
        ...
        {/* Expansion divisions — coming soon */}
        <Reveal delay={0.1}>
          <div className="mt-14 flex flex-col items-start gap-5 border-t border-white/10 pt-10 sm:flex-row sm:items-center sm:justify-between">
            <span className="font-display text-kicker uppercase text-mist/70">
              {SERVICES.comingSoon.label}
            </span>
            <ul className="flex flex-wrap gap-3">
              {SERVICES.comingSoon.items.map((c) => (
                <li
                  key={c}
                  className="rounded-full border border-white/15 px-4 py-1.5 font-display text-sm text-mist"
                >
                  {c}
                </li>
              ))}
